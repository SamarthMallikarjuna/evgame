...
const CLOUD_CONFIG = {
    primaryUrl: 'https://extendsclass.com/api/json-storage/bin/bcfaada',
    backupUrl: 'https://extendsclass.com/api/json-storage/bin/eaacadb',
    syncing: false,
    lastSyncTime: 0
};
...
async function syncCloudDatabase() {
    ...
    try {
        let remoteData = null;
        const buildUrl = (baseUrl) => `${baseUrl}?t=${Date.now()}`;
        try {
            const res = await fetch(buildUrl(CLOUD_CONFIG.primaryUrl), { headers: { 'Security-Key': 'no-cache' } });
            if (res.ok) {
        ...
        if (!remoteData || (!remoteData.accounts && !remoteData.leaderboard)) {
            try {
                const res = await fetch(buildUrl(CLOUD_CONFIG.backupUrl), { headers: { 'Security-Key': 'no-cache' } });
                if (res.ok) {
                    const json = await res.json();
                    remoteData = json.data || json;
                }
        ...
        if (remoteData && (Array.isArray(remoteData.accounts) || Array.isArray(remoteData.leaderboard))) {
            ...
            const remoteAccCount = Array.isArray(remoteData.accounts) ? remoteData.accounts.length : 0;
            const remoteLbCount = Array.isArray(remoteData.leaderboard) ? remoteData.leaderboard.length : 0;

            const needsPush = mergedAccounts.length > remoteAccCount ||
                              mergedLb.length > remoteLbCount;

            if (needsPush) {
                await pushCloudDatabase(mergedAccounts, mergedLb);
            }
...
async function pushCloudDatabase(accountsList, leaderboardList) {
    ...
    const bodyPayload = JSON.stringify({
        name: 'EcoRunner_Global_Production_Database_v2',
        accounts: accs,
        leaderboard: lb
    });
    ...
    try {
        await Promise.allSettled([
            fetch(CLOUD_CONFIG.primaryUrl, { method: 'PUT', headers, body: bodyPayload }),
            fetch(CLOUD_CONFIG.backupUrl, { method: 'PUT', headers, body: bodyPayload })
        ]);
    } catch (e) {}
}
...
window.addEventListener('focus', syncCloudDatabase);
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) syncCloudDatabase();
});
